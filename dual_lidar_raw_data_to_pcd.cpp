...
struct PointCloudTask {
    ...
    std::string timestamp;
    size_t valid_points;
    int sensor_id; // 0 for LiDAR 7502, 1 for LiDAR 7504
    std::chrono::system_clock::time_point time_point;
};

// Structure to hold paired point clouds for stitching
...
// Stitching thread to pair and merge point clouds
void stitch_thread_func(StitchingQueue& queue1, StitchingQueue& queue2, MergeQueue& merge_queue,
                        const std::string& output_dir, std::ofstream& packet_log) {
    std::vector<PointCloudTask> buffer1, buffer2;
    ...
    while (g_running) {
        ...
        for (auto it1 = buffer1.begin(); it1 != buffer1.end();) {
            ...
            for (auto it2 = buffer2.begin(); it2 != buffer2.end();) {
                ...
                if (time_diff <= max_time_diff.count()) {
                    // Pair found, apply stitching
                    PointCloudPair pair;
                    pair.cloud1 = it1->cloud;
                    pair.cloud2 = it2->cloud;
                    pair.timestamp = it1->timestamp; // Use LiDAR 7502's timestamp
                    apply_stitch_transformation(pair.cloud2); // Align LiDAR 7504 to 7502
                    *pair.cloud1 += *pair.cloud2; // Merge
                    apply_transformation(pair.cloud1); // Apply ground plane transform
                    // Apply Z-based filtering
                    pair.cloud1 = filter_by_height(pair.cloud1, 0.25f, 3.0f);
                    // Apply clustering and add bounding boxes
                    pair.cloud1 = cluster_and_add_boxes(pair.cloud1, 0.5f, 0.08f, 10);
                    pair.valid_points = pair.cloud1->size();

                    PointCloudWriteTask write_task;
                    write_task.cloud = pair.cloud1;
                    write_task.valid_points = pair.valid_points;
                    write_task.timestamp = pair.timestamp;
                    write_task.filename = output_dir + "/merged_scan_" + pair.timestamp + ".pcd";
                    merge_queue.push(std::move(write_task));

                    packet_log << "[" << it1->timestamp << "] Paired Sensor 1 (" << it1->valid_points
                               << " points) with Sensor 2 (" << it2->valid_points
                               << " points), merged: " << pair.valid_points << " points" << std::endl;
...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0]
                  << " <sensor1_hostname> <sensor2_hostname> <output_dir>" << std::endl;
        return 1;
    }
    ...
    try {
        ...
        ouster::sensor::SensorClient* client = new ouster::sensor::SensorClient({sensor1, sensor2});

        auto infos = client->get_sensor_info();
        if (infos.size() < 2) {
            std::cerr << "Failed to get sensor info for both sensors. Got info for " << infos.size() << " sensors." << std::endl;
            delete client;
            return 1;
        }
        ...
        ouster::sensor::StitchingQueue stitch_queue1, stitch_queue2;
        ouster::sensor::MergeQueue merge_queue;
        std::thread stitch_thread(ouster::sensor::stitch_thread_func, std::ref(stitch_queue1),
                                 std::ref(stitch_queue2), std::ref(merge_queue),
                                 std::ref(output_path.string()), std::ref(packet_log));
        ...
        while (ouster::sensor::g_running) {
            ...
            if (ev.type == ouster::sensor::ClientEvent::Packet) {
                ...
                auto& scan = is_sensor1 ? scan1 : scan2;
                auto& batcher = is_sensor1 ? batcher1 : batcher2;
                auto& stitch_queue = is_sensor1 ? stitch_queue1 : stitch_queue2;
                auto& lut = is_sensor1 ? lut1 : lut2;
                auto& last_scan_time = is_sensor1 ? last_scan_time1 : last_scan_time2;
                auto& packet_count = is_sensor1 ? packet_count1 : packet_count2;
                ...
                if (ev.packet().type() == ouster::sensor::PacketType::Lidar) {
                    ...
                    if (batcher(lidar_packet, scan)) {
                        ...
                        if (scan.complete(is_sensor1 ? info1.format.column_window : info2.format.column_window) || duration_ms > 120) {
                            packet_log << "Sensor " << (is_sensor1 ? "1" : "2") << " packets received: " << packet_count << std::endl;
                            ...
                            if (scan.w != 1024) {
                                ...
                                packet_log << "[" << timestamp.str() << "] Sensor " << (is_sensor1 ? "1" : "2")
                                           << " incomplete scan, columns: " << scan.w << ", packets: " << packet_count << ", skipped" << std::endl;
                                scan = ouster::LidarScan{is_sensor1 ? info1 : info2};
                                last_scan_time = now;
                                packet_count = 0;
                                continue;
                            }
                            ...
                            stitch_task.sensor_id = is_sensor1 ? 0 : 1;
